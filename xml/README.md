...
数据标记语言设计目的是从文本中提取格式化数据.相比于手动提取方式，突出优势为当原文有变动时能实现最小化改动成本，部分改动要求很多以至于更多角色可以承担改动工作(比如：产品修改需求文档后，研发无需参与，对应的API文档都能自动同步修改)
***提取数据的一般流程为：***
...
***数据标记语言工作量：***
数据标记语言主要用于从普通文本中提取结构化数据，目前这种工作主要是人工手动处理。一般流程为：
1. 设计目标格式化数据结构
2. 书写目标格式化数据模板
...
2. 将json数据转为目标格式数据比手动复制填充模板复杂很多
3. 增加标记比复制数据心智负担大
...
，此时会带来一定优势：
1. 能增量式的标记数据，而手动提取往往需要一气呵成，降低工作难度
2. 有利于从不同角度思考问题，而不限于结构化数据角度
3. 当原文修改时，其相关链条也需要同步修改时，改动成本非常低(近似线性关系)，而这种情况对"手动提取方式"就是天灾，改动成本明显上升(近似指数关系)
