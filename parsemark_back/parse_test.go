...
func GetRecords() parsemark.Records {
	...
	records, err := parsemark.Parse(source)
	if err != nil {
		panic(err)
	}
	return records
...
func TestGetRefs(t *testing.T) {
	...
	parsemark.ResolveRef(refRecords)
	fmt.Println(refRecords.String())
}

func TestMerge(t *testing.T) {
	records := GetRecords()
	newRecords, err := parsemark.MergeRecords(records...)
	if err != nil {
		panic(err)
	}
...
func TestRecordString(t *testing.T) {
	records := GetRecords()
	out := records.FilterByKV(parsemark.KV{Key: parsemark.KEY_DB, Value: "doc"}).String()
	fmt.Println(out)
}
